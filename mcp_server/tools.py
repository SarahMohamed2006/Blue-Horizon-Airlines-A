...
def assign_aircraft(data: AssignAircraftInput):
    ...
    try:
        ...
        cursor.execute("""
            SELECT flight_id, aircraft_id, status
            FROM Flights
            WHERE flight_id = ?
        """, (flight_id,))
        ...
        if old_aircraft_id and old_aircraft_id != aircraft_id:
            cursor.execute("""
                UPDATE Aircraft
...
def reschedule_flight(
    ...
    try:
        ...
        if employee["role"] not in ("Operations Manager", "Dispatcher"):
            return {
                "error": "Unauthorized. Only Operations Manager or Dispatcher can reschedule flights."
            }
...
async def cancel_flight(
    ...
    try:
        ...
        try:
            ...
            return {
                "error": f"This action requires a client with elicitation support to confirm cancellation: {exc}"
            }
...
def complete_maintenance(
    ...
    try:
        ...
        if employee["role"] not in (
            ...
            return {
                "error": "Unauthorized. Only Maintenance Engineer or Operations Manager can close out maintenance."
            }
                    ...
                    SamplingMessage(
                        ...
                        content=TextContent(
                            ...
                            text=(
                                ...
                                "In 2-3 sentences, assess the operational risk of "
                                "this decision and note anything the stated reason "
                                "does not address."
                            ),
                        ),
                    )
...
async def resolve_operational_issue(
    ...
    if decision == "Cancel Flight":
        ...
        return {
            "error": "Reschedule requires new_departure/new_arrival. Call reschedule_flight directly."
        }
    ...
    elif decision == "Delay Flight":
        return {
            "error": "Delay Flight requires delay details and is not implemented."
        }
